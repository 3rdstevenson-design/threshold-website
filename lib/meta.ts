...
export async function checkContainerStatus(containerId: string): Promise<'FINISHED' | 'IN_PROGRESS' | 'ERROR'> {
  ...
  // Only an explicit IN_PROGRESS keeps the post waiting. Anything else —
  // ERROR, EXPIRED, an error payload with no status_code (expired token,
  // deleted container), or an unknown code — is treated as a failure so the
  // post can't sit in 'processing' forever. (A thrown fetch = network error,
  // which the caller handles separately and does NOT treat as failure.)
  if (json.status_code === 'IN_PROGRESS') return 'IN_PROGRESS';
  console.error(
    `checkContainerStatus(${containerId}): non-progress response treated as ERROR: ${JSON.stringify(json).slice(0, 300)}`,
  );
  return 'ERROR';
}
